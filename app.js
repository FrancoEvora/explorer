(() => {
  ...
  const LEGACY_STORAGE_KEY = 'caderno-campo-state-v1';
  ...
  async function bootstrap() {
    if (!window.ExplorerCloud) {
      console.error('Explorer Cloud indisponível.');
      return;
    }
    await window.ExplorerCloud.start({
      onAuthenticated: async () => {
        const remoteState = await window.ExplorerCloud.loadState(state);
        ...
        uploadPendingMedia().catch((error) => console.warn('Mídias pendentes não enviadas', error));
      },
      onSignedOut: () => {
        stopSOS();
        stopGpsWatch();
  ...
  function loadState() {
    try {
      ...
      if (raw && !localStorage.getItem(STORAGE_KEY)) localStorage.setItem(STORAGE_KEY, JSON.stringify(normalized));
      return normalized;
    } catch (error) {
  ...
  function saveState() {
    persistLocalState();
    window.ExplorerCloud?.scheduleSync(state);
    if (navigator.onLine) uploadPendingMedia().catch((error) => console.warn('Falha no envio de mídia pendente', error));
  }
  ...
  function exportJson() {
    const payload = {
      ...
      app: 'Explorer',
      note: 'Backup estruturado do Explorer. A base principal e as mídias ficam sincronizadas no Supabase.',
      state
    };
    ...
    anchor.href = url;
    anchor.download = `explorer-backup-${new Date().toISOString().slice(0, 10)}.json`;
    anchor.click();
  ...
  function exportCsv() {
    ...
    downloadText(`explorer-observacoes-${new Date().toISOString().slice(0, 10)}.csv`, csv, 'text/csv;charset=utf-8');
  }
  ...
  function exportGpx() {
    ...
    const gpx = `<?xml version="1.0" encoding="UTF-8"?>\n<gpx version="1.1" creator="Explorer" xmlns="http://www.topografix.com/GPX/1/1">\n  <metadata><name>${xmlEscape(name)}</name><time>${new Date().toISOString()}</time></metadata>\n${waypoints ? waypoints + '\n' : ''}  <trk><name>${xmlEscape(name)}</name><trkseg>\n${trkpts}\n  </trkseg></trk>\n</gpx>\n`;
    const safeName = slugify(name) || 'trilha';
    downloadText(`explorer-${safeName}-${new Date().toISOString().slice(0, 10)}.gpx`, gpx, 'application/gpx+xml;charset=utf-8');
  }
  ...
  async function clearAllData() {
    const ok = confirm('Apagar definitivamente trilhas, observações, riscos e mídias da nuvem e deste aparelho? Esta ação não pode ser desfeita.');
    if (!ok) return;
    stopSOS();
    stopGpsWatch();
    stopTimer();
    ...
    try {
      await window.ExplorerCloud?.clearCloudData();
    } catch (error) {
      console.warn('Falha ao limpar a nuvem', error);
      alert('Não foi possível concluir a exclusão na nuvem. Verifique a internet e tente novamente.');
      return;
    }
    state = defaultState();
    persistLocalState();
    try {
      ...
      console.warn('Falha ao limpar mídia local', error);
    }
    hydrateSettingsForm();
    renderAll();
    toast('Dados apagados da nuvem e deste aparelho.');
  }
  ...
  function bindStatusWatchers() {
    ...
    window.addEventListener('online', () => {
      updateOnlineStatus();
      window.ExplorerCloud?.scheduleSync(state, 200);
      uploadPendingMedia().catch((error) => console.warn('Falha ao retomar uploads', error));
    });
  ...
  function createGeneratedToneUrl({ duration = 2, frequencies = [900], segment = 0.5, amplitude = 0.65 } = {}) {
    ...
    for (let i = 0; i < sampleCount; i += 1) {
      ...
      const frequency = frequencies[Math.floor(t / segment) % frequencies.length];
      const attack = Math.min(1, t / 0.025);
      const release = Math.min(1, (duration - t) / 0.025);
      const envelope = Math.max(0, Math.min(attack, release));
      const sample = Math.max(-1, Math.min(1, amplitude * envelope * Math.sin(2 * Math.PI * frequency * t)));
  ...
  function setupSirenAudio() {
    ...
    try {
      if (audio && !audio.src) audio.src = createGeneratedToneUrl({ duration: 2.4, frequencies: [850, 1250], segment: 0.6, amplitude: 0.72 });
      if (unlockAudio && !unlockAudio.src) unlockAudio.src = createGeneratedToneUrl({ duration: 0.08, frequencies: [900], segment: 0.08, amplitude: 0.08 });
    } catch (error) {
      console.warn('Não foi possível gerar o arquivo local da sirene', error);
    }
    if (audio) {
      audio.loop = true;
      audio.preload = 'auto';
  ...
  async function shareText(text) {
    if (navigator.share) {
      try {
        await navigator.share({ title: 'Explorer', text });
        return;
      } catch (error) {
  ...
  async function openMediaViewer(media, title, obs) {
    ...
    download.href = url;
    download.download = media.name || 'midia-explorer';
  ...
  async function storeMediaFile(file, kind) {
    ...
    const storagePath = await window.ExplorerCloud?.uploadFile(file, id) || '';
    return { id, kind, name: file.name, type: file.type, size: file.size, createdAt: record.createdAt, storagePath };
  }
  ...
  async function getLocalMediaRecord(id) {
    try {
      ...
      console.warn('Falha ao ler mídia local', error);
      return null;
    }
  }

  function allMediaDescriptors() {
    const items = [];
    state.observations.forEach((obs) => {
      (obs.media || []).forEach((media) => items.push(media));
      (obs.professionalPhotos || []).forEach((media) => items.push(media));
    });
    state.risks.forEach((risk) => (risk.media || []).forEach((media) => items.push(media)));
    return items;
  }

  function findMediaDescriptor(id) {
    return allMediaDescriptors().find((media) => media.id === id) || null;
  }

  async function uploadPendingMedia() {
    if (pendingMediaSync || !navigator.onLine || !window.ExplorerCloud?.getUser?.()) return;
    pendingMediaSync = true;
    let changed = false;
    try {
      const pending = allMediaDescriptors().filter((media) => media.id && !media.storagePath);
      for (const media of pending) {
        const record = await getLocalMediaRecord(media.id);
        if (!record?.blob) continue;
        const uploadFile = record.blob instanceof File
          ? record.blob
          : new File([record.blob], media.name || record.name || 'arquivo', { type: media.type || record.type || 'application/octet-stream' });
        const storagePath = await window.ExplorerCloud.uploadFile(uploadFile, media.id);
        if (storagePath) {
      ...
      if (changed) {
        persistLocalState();
        await window.ExplorerCloud.syncNow(state);
        renderAll();
      }
    } finally {
  ...
  async function loadMediaUrl(id) {
    const descriptor = findMediaDescriptor(id);
    const local = await getLocalMediaRecord(id);
    if (local?.blob) return URL.createObjectURL(local.blob);
    if (descriptor?.storagePath) {
      return await window.ExplorerCloud?.getSignedMediaUrl(descriptor.storagePath) || '';
    }
    return '';
  }
