...
a="  elapsed=progress();active=false;bus.gain.cancelScheduledValues(ctx.currentTime);bus.gain.setValueAtTime(0,ctx.currentTime);\n  if(source){const s=source;source=null;s.onended=null;try{s.stop();}catch{}s.disconnect();}"
b="  // Suspend the shared context without destroying its loop graph.\n  // The audio clock pauses with it, so resuming continues the same musical phrase.\n  active=false;bus.gain.cancelScheduledValues(ctx.currentTime);bus.gain.setValueAtTime(0,ctx.currentTime);"
assert a in score
...
s=s.replace("aud=p=>p.evaluate(()=>SolarisSound.snapshot());", "aud=(p,measure=false)=>p.evaluate(m=>SolarisSound.snapshot(m),measure);")
s=s.replace("max=Math.max(max,(await aud(p)).rms);", "max=Math.max(max,(await aud(p,true)).rms);")
s=s.replace('()=>SolarisSound.snapshot().','()=>SolarisSound.snapshot(false).')
s=s.replace("await p.waitForTimeout(500);assert(await maxRMS(p)>0.00001,'Music alone", "await p.waitForFunction(()=>SolarisSound.snapshot(false).musicTrack?.ready,null,{timeout:15000});await p.waitForTimeout(500);assert(await maxRMS(p)>0.00001,'Music alone")
s=s.replace("async function shot(p,name){await p.screenshot({path:path.join(OUT,name+'.png')});}","async function shot(p,name){await p.screenshot({path:path.join(OUT,name+'.png'),timeout:5000});}\nfunction checkpoint(name){console.log('CHECKPOINT '+name);fs.writeFileSync(path.join(OUT,'checkpoint.txt'),name);}\nasync function tapControl(p,id){const el=p.locator('#'+id);await el.waitFor({state:'visible'});const r=await el.boundingBox();const point={x:r.x+r.width/2,y:r.y+r.height/2};assert(await p.evaluate(({x,y,id})=>document.elementFromPoint(x,y)?.closest('#'+id)!=null,{...point,id}),'Control must be hit-testable');if(await p.evaluate(()=>navigator.maxTouchPoints>0))await p.touchscreen.tap(point.x,point.y);else await p.mouse.click(point.x,point.y);}")
...
("await p.waitForFunction(()=>SolarisSound.snapshot(false).state==='running');await p.evaluate", "await p.waitForFunction(()=>SolarisSound.snapshot(false).state==='running');checkpoint('unmute-complete');await p.evaluate"),
("await p.waitForFunction(()=>SolarisSound.snapshot(false).state==='suspended');assert.equal", "await p.waitForFunction(()=>SolarisSound.snapshot(false).state==='suspended');checkpoint('pagehide-suspended');assert.equal"),
("await p.waitForFunction(()=>SolarisSound.snapshot(false).musicTrack.active);await p.waitForTimeout", "await p.waitForFunction(()=>SolarisSound.snapshot(false).musicTrack.active);checkpoint('pagehide-music-active');await p.waitForTimeout"),
...
assert a in s
s=s.replace(a,b).replace("await p.click('#soundToggle')","await tapControl(p,'soundToggle')")
p.write_text(s)
print('Music loop graph retained across suspension; native touch verifies user-activated iPhone resume.')
