create table private.solaris_lot_requests (
 id uuid primary key,
 unit_id uuid not null references public.crm_inventory_units(id),
 crm_record_id uuid not null references public.crm_records(id),
 payload_hash text not null,
 created_at timestamptz not null default now()
);
create index solaris_lot_requests_dedupe_idx on private.solaris_lot_requests(payload_hash,created_at desc);
alter table private.solaris_lot_requests enable row level security;
revoke all on private.solaris_lot_requests from public,anon,authenticated;
grant select,insert on private.solaris_lot_requests to service_role;
...
begin
 ...
 select fallback_owner_user_id into owner_key from crm_private.public_agent_experiences where organization_id=cfg.organization_id and project_id=cfg.project_id and active order by created_at desc limit 1;
