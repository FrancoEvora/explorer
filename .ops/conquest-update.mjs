...
export const CONQUEST_VERSION='solaris-conquista-v2.2.1';
export function installConquest(outDir='solaris-dist') {
  const out=path.resolve(outDir),source=path.resolve('site/conquista');
  const files=['index.html','conquista.css','paths.js','music.js','audio.js','input.js','conquista.js'];
  ...
  for(const f of ['paths.js','music.js','audio.js','input.js','conquista.js'])new vm.Script(texts[f]);
  ...
  manifest.game={version:CONQUEST_VERSION,path:'/jogo/',levels:15,chapters:5,maxStars:45,rendering:'2D image-based',rewards:'virtual only',currency:'virtual suns; no cash value',storage:'local browser plus manual passport export/import',serverValidated:false,physicalPrizes:false,locations:9,freeExploration:true,exploration:{places:9,firstVisitXP:20,firstVisitSuns:15,collectionXP:100,collectionSuns:100,duplicateRewards:false},audio:{music:'Caminhos de Sol',musicLoop:true,engine:'Web Audio',gestureRequired:true,ambient:true,effects:true,volumeControls:true},input:{nativeTouch:true,inlineIcons:true,guidedFirstPhase:true},sourceSHA256:crypto.createHash('sha256').update(files.map(f=>texts[f]).join('')).digest('hex')};
