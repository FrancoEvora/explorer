...
function create(ctx,destination){
 ...
 bus.gain.value=0;filter.type='lowpass';filter.frequency.value=6000;filter.Q.value=.3;filter.connect(bus);bus.connect(destination);
 ...
 function stop(){
  // Suspend the shared context without destroying its loop graph.
 ...
 function snapshot(){
  ...
  return{title:'Caminhos de Sol',bpm:BPM,bars:16,active,ready:!!buffer,failed,scoreNotes:events.length,playedNotes:loops*events.length+events.filter(e=>e<=at).length,loops,bar:Math.min(16,Math.floor(at/(4*BEAT))+1),voices:source?1:0};
 }
