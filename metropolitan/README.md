...
Plataforma visual em português com navegação pela implantação aprovada, cinco pontos de interesse, nove quadras, percurso guiado, ampliação, arraste, gestos de pinça, navegação por teclado, apresentação e galeria de três novas renderizações.
...
A publicação inicial usa `/metropolitan` no projeto Vercel já conectado a `FrancoEvora/explorer`, branch `solaris-imersivo-mobile`. O build do projeto pai copia apenas os sete arquivos públicos do Metropolitan, acrescenta a base `/metropolitan/` ao HTML e verifica que os seis arquivos existentes do Solaris continuam com o mesmo hash.
